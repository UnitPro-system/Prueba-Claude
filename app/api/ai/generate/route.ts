...
export async function POST(req: Request) {
  try {
    ...
    // Verificar saldo — NOTE: patrón SELECT + UPDATE tiene race condition teórica (TOCTOU).
    // Mitigado con validación de negocio ownership. Fix atómico vía DB function en v2.
    ...
    // Descontar 1 UC después de generación exitosa
    await supabaseAdmin.from('unitcoins_balance').upsert(
      {
