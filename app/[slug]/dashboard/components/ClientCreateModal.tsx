...
export default function ClientCreateModal({ open, agencyId, onClose, onCreated }: ClientCreateModalProps) {
  ...
  const stepTitle: Record<string, string> = {
    choice: 'Nuevo Cliente', blocks: 'Elegí los bloques',
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            {step !== 'choice' && (
              <button onClick={() => setStep('choice')} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 transition-colors">
                <ArrowLeft size={16} />
              </button>
            )}
            <div>
        ...
        <div className="p-6">
          ...
          {step === 'choice' && (
            <div className="space-y-3">
              {[
                ...
                <button key={opt.step}
                  onClick={() => { if (opt.step === 'blocks') setSelBlocks(['landing']); setStep(opt.step); }}
          ...
          {step === 'template' && (
            <div className="space-y-3">
              {TEMPLATES.map(t => (
                <button key={t.id} onClick={() => { setSelBlocks(t.blocks); setStep('form'); }}
