...
// Tabla completa del legacy: deduplicado por email, expandible en móvil,
// WhatsApp + Email (dispara modal del shell) en desktop.
...
export default function CrmAdmin({ sharedData }: BlockAdminProps) {
  const { turnos, openContactModal } = sharedData;

  // Deduplicar — mismo algoritmo que el legacy
  const clientes = turnos.filter((obj: any, idx: number, self: any[]) =>
  ...
  return (
    <div className="animate-in fade-in">
      <header className="mb-6">
        ...
        <p className="text-zinc-500 text-sm">{clientes.length} clientes únicos registrados.</p>
      </header>
              ...
              <tbody className="divide-y divide-zinc-50">
                {clientes.map((c: any) => (
                  <tr key={c.id} className="group hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-zinc-500">{c.servicio || "General"}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          onClick={() => openContactModal(c.cliente_email, c.cliente_nombre)}
