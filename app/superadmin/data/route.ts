...
// Devuelve: lista de agencias con cantidad de negocios y fecha de registro.
// Requiere la cookie de sesión del superadmin.
...
export async function GET() {
  ...
  const { data: agencies, error } = await supabase
    .from('agencies')
    .select('id, name, nombre_agencia, email, slug, created_at')
  ...
  // Para cada agencia, contar sus negocios
  ...
  // Config de plataforma (precios UC y tasa de cambio)
