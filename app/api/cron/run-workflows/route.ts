        ...
        switch (wf.recipe_id) {
          ...
          case 'reminder_24h': {
            ...
            for (const t of turnos ?? []) {
              ...
              if (res.success) {
                sent++;
                await supabaseAdmin.from('turnos').update({ recordatorio_enviado: true }).eq('id', t.id);
              } else { errors++; }
            }
            break;
          }
          ...
          case 'post_visit_review': {
            ...
            for (const t of turnos ?? []) {
              ...
              const { data: row } = await supabaseAdmin
                .from('turnos').select('metadata').eq('id', t.id).single();
              if (row?.metadata?.review_sent) continue;
              ...
              if (res.success) {
                sent++;
                await supabaseAdmin.from('turnos').update({
                  metadata: { ...(row?.metadata || {}), review_sent: true }
                }).eq('id', t.id);
              } else { errors++; }
            }
            break;
