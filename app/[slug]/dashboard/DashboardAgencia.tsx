...
export default function DashboardAgencia() {
  ...
  const [quickEditClient, setQuickEditClient]   = useState<any>(null);
  ...
  const openQuickEdit = (cliente: any) => {
    setQuickEditClient(cliente);
    setQuickEditData({
  ...
  const handleSaveQuickEdit = async () => {
    ...
    const { error } = await supabase.from("negocios").update({
      nombre:    quickEditData.nombre,
      whatsapp:  quickEditData.whatsapp,
      direccion: quickEditData.direccion,
    }).eq("id", quickEditClient.id);

    if (error) {
      alert("Error al guardar: " + error.message);
    } else {
      setQuickEditSaved(true);
      // Actualizar lista local sin re-fetch
      setClientes(prev => prev.map(c =>
        c.id === quickEditClient.id
          ? { ...c, nombre: quickEditData.nombre, whatsapp: quickEditData.whatsapp, direccion: quickEditData.direccion }
          : c
      ));
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clientes.map(cliente => (
            <div key={cliente.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-[#b0c97d] transition-all p-6 flex flex-col justify-between group relative">
              ...
              {/* Botón edición rápida */}
              <button onClick={() => openQuickEdit(cliente)}
                className="absolute top-4 right-12 p-2 text-slate-300 hover:text-[#577a2c] hover:bg-[#577a2c]/10 rounded-full transition-all">
                <Pencil size={16} />
              </button>
              <div>
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-md"
                  style={{ backgroundColor: quickEditClient.color_principal || "#577a2c" }}>
                  {quickEditClient.nombre?.charAt(0) || "N"}
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 text-sm">Editar cliente</h3>
                  <p className="text-xs text-slate-400">{quickEditClient.email}</p>
                </div>
              </div>
